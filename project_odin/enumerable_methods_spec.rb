...
describe "my_each" do
  it "works with block" do
    a=0
    expect([1,2,3,4,5].my_each{ |n| a+=n }).to eq([1,2,3,4,5])
    expect(a).to eq(15)
  end
end
...
describe "my_each_with_index" do
  it "works with block" do
    a=0
    expect([1,1,1,1,1].my_each_with_index{ |n,i| a+=(i+n) }).to eq([1,1,1,1,1])
    expect(a).to eq(15)
  end
end
...
describe "my_all?" do
  ...
  it "works without block" do
    ...
    expect(["bacon", "toaster"].my_all?).to eq(true)
  end
end
...
describe "my_inject" do
  ...
  it "works with block + arg" do
    expect([1,2,3,4,5].my_inject(1){|product,n| product * n}).to eq(120)
  end
  it "works with text" do
    expect(["Big","World"].my_inject("Hello"){|sentence,word| sentence + " " + word}).to eq("Hello Big World")
  end
end